...
fn process_input(lines: Lines<BufReader<File>>) -> u32 {
    ...
    for line in lines {
        result += process_line(preprocess_line(
            line.expect("failed to read input file line"),
        ));
    }
...
mod tests {
    ...
    #[test]
    fn test_calibrate() {
        let file_path = "inputs\\day01p02-example.txt";
        let file = File::open(file_path).expect("Failed to read input file.");
        let reader = BufReader::new(file);
        assert_eq!(process_input(reader.lines()), 281);
    }
}
